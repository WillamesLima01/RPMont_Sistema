...
const GraficoCargaHorariaEquino = () => {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      try {
        ...
        console.error('Erro ao carregar gráfico de carga horária:', error);
        if (chartRef.current) {
  ...
  useEffect(() => {
    if (mesSelecionado) {
      Promise.all([axios.get('/escala'), axios.get('/equino')])
  ...
  const montarGrafico = (escalas, equinos, mes) => {
    ...
    chart.yScale().maximum(180);
    chart.yScale().ticks({ interval: 30 });

    chart.tooltip().positionMode('point');
    chart.tooltip().format('Carga Horária: {%Value}h');
